...
function exportToJsonFile(jsonData) {
    ...
    let exportFileDefaultName = 'cardinal.json';
...
function saveProfile() {
    ...
    var myToast = Toastify({
        text: "Profile Saved",
        duration: 5000,
        backgroundColor: "linear-gradient(#7B4FFF, #7B4FFF, #7B4FFF)",
        offset: {
            x: 0,
            y: 100
        }
...
function keyAuth() {
    ...
    chrome.instanceID.getID(function(info) {
        ...
        myHeaders.append("Authorization", "Bearer ak_ptFr9KEHto9ymis2szte");
        ...
        fetch("http://dashboard.cardinalscripts.com/api/v1/activations", requestOptions)
            .then(response => response.text())
            .then(
                result => {
                    var resultDTC = (JSON.parse(result))
                    console.log(resultDTC)
                    if (resultDTC['success']) {
                        chrome.storage.local.set({
                            activated: true,
                            key: key,
                            activation_token: resultDTC['activation_token']
                        });
...
function unbind() {
    ...
    chrome.storage.local.get(function(items) {
        ...
        fetch(`http://dashboard.cardinalscripts.com/api/v1/activations/${items.activation_token}`, requestOptions)
            .then(response => response.text())
            .then(result => {
...
document.getElementById('dwb').addEventListener("click", function() {
    ...
    sendSuccess("Webhook Test", "CardinalScripts.com", 'https://media.discordapp.net/attachments/720807063285923862/720818962530697216/cardinalhighres.png', "test", document.getElementById('dhook').value)
    settingSavedToast()
});
    ...
    if (!(customHook === 'NA')) {
        ...
        xmlhttp3.send(JSON.stringify({
            "embeds": [{
                ...
                "footer": {
                    "icon_url": "https://cdn.shopify.com/s/files/1/0406/4103/6444/products/cardinalwhite_540x.png?v=1602036648",
                    "text": "Cardinal Scripts"
                },
                "thumbnail": {
                    "url": thumbnail
                },
            }],
            "username": "Cardinal Success",
            "avatar_url": "https://cdn.shopify.com/s/files/1/0406/4103/6444/products/cardinalwhite_540x.png?v=1602036648"
        }));
    }
